...
public class Pet implements ConfigurationSerializable {
    ...
    public void setLastWater(long t) {
        lastWater = t;
    }

    public long getLastWashed() {
        return lastWashed;
    }

    public void setLastWashed(long t) {
        lastWashed = t;
    }
    ...
    public void setLastHourly(long t) {
        lastHourly = t;
    }

    public long getLastWalkQuest() {
        return lastWalkQuest;
    }

    public void setLastWalkQuest(long t) {
        lastWalkQuest = t;
    }

    public boolean isWalkQuestReadyNotified() {
        return walkQuestReadyNotified;
    }

    public void setWalkQuestReadyNotified(boolean v) {
        walkQuestReadyNotified = v;
    }
    ...
    public int getUpZoeken() {
        return upZoeken;
    }

    public int getUpGrinden() {
        return upGrinden;
    }

    public int getUpWater() {
        return upWater;
    }

    public int getUpEten() {
        return upEten;
    }

    public int getUpUurloon() {
        return upUurloon;
    }

    public int getUpSnelheid() {
        return upSnelheid;
    }

    public int foodIntervalMinutes() {
        ...
        if (inst != null) {
            base = inst.getConfig().getInt("feed.base-cooldown-minutes", 10);
            extraPerLevel = inst.getConfig().getInt("feed.extra-minutes-per-eten-level", 1);
        }

        return Math.max(1, base + (upEten - 1) * extraPerLevel);
    }
    ...
    public int waterIntervalMinutes() {
        ...
        if (inst != null) {
            base = inst.getConfig().getInt("drink.base-cooldown-minutes", 10);
            extraPerLevel = inst.getConfig().getInt("drink.extra-minutes-per-water-level", 1);
        }

        return Math.max(1, base + (upWater - 1) * extraPerLevel);
    }
    ...
    public int lootIntervalMinutes() {
        ...
        int base = 30;
        int reducePerLevel = 2;
        int minimum = 5;

        if (inst != null) {
            base = inst.getConfig().getInt("loot.base-minutes", 30);
            reducePerLevel = inst.getConfig().getInt("loot.reduce-minutes-per-zoeken-level", 2);
            minimum = inst.getConfig().getInt("loot.minimum-minutes", 5);
        }

        int reduced = base - (upZoeken - 1) * reducePerLevel;

        return Math.max(minimum, reduced);
    }

    public boolean isHungry() {
        return System.currentTimeMillis() - lastFed >= foodIntervalMinutes() * 60_000L;
    }

    public boolean isThirsty() {
        return System.currentTimeMillis() - lastWater >= waterIntervalMinutes() * 60_000L;
    }
